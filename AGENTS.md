...
- `npm run android:move-outputs` refuses to overwrite, and `native-debug-symbols.zip` is not versioned in its name — so a previous release's copy left in `dist/` aborts the script. The APKs and `.aab` are moved before it reaches that point, so the release is unaffected; clear the stale file if you want a clean run.
- Tag `v<version>` and create the release with `--draft`; the owner reviews and publishes it.
- **Pass `--repo ixPansi/Mazika` to every `gh` command.** The `upstream` remote points at `zyrouge/symphony`, and `gh` may resolve there instead — `gh release create` then fails claiming the tag was never pushed.
